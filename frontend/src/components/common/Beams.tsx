...
function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: ExtendMaterialConfig): THREE.ShaderMaterial {
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    uniforms[key] = u !== null && typeof u === 'object' && 'value' in (u as any) ? (u as THREE.IUniform) : { value: u } as THREE.IUniform;
  });
  let vert = `${cfg.header}\n${cfg.vertexHeader ?? ''}\n${baseVert}`;
  let frag = `${cfg.header}\n${cfg.fragmentHeader ?? ''}\n${baseFrag}`;
  ...
  return new THREE.ShaderMaterial({ defines: { ...baseDefines }, uniforms, vertexShader: vert, fragmentShader: frag, lights: true, fog: !!(cfg.material as any)?.fog });
}
...
const Beams: FC<BeamsProps> = ({ beamWidth=2, beamHeight=15, beamNumber=12, lightColor='#ffffff', speed=2, noiseIntensity=1.75, scale=0.2, rotation=0 }) => {
  ...
  const beamMaterial = useMemo(() => extendMaterial(THREE.MeshStandardMaterial, {
    ...
    material: { fog: true },
    uniforms: { diffuse: new THREE.Color(...hexToRGB('#000000')), time: { value: 0 }, roughness: 0.3, metalness: 0.3, uSpeed: { value: speed }, envMapIntensity: 10, uNoiseIntensity: noiseIntensity, uScale: scale },
...
function createStackedPlanes(n: number, width: number, height: number, spacing: number, heightSegments: number) {
  const geometry = new THREE.BufferGeometry();
  const numV = n*(heightSegments+1)*2, numF = n*heightSegments*2;
  const positions = new Float32Array(numV*3), indices = new Uint32Array(numF*3), uvs = new Float32Array(numV*2);
  let vOff=0, iOff=0, uvOff=0;
  const totalW = n*width+(n-1)*spacing, xBase = -totalW/2;
  for (let i=0;i<n;i++) {
    ...
    for (let j=0;j<=heightSegments;j++) {
      ...
      uvs.set([uvX,j/heightSegments+uvY,uvX+1,j/heightSegments+uvY],uvOff);
...
const MergedPlanes = forwardRef<THREE.Mesh<THREE.BufferGeometry, THREE.ShaderMaterial>, { material: THREE.ShaderMaterial; width: number; count: number; height: number }>(
  ({ material, width, count, height }, ref) => {
    ...
    const geometry = useMemo(() => createStackedPlanes(count, width, height, 0, 100), [count, width, height]);
...
const DirLight: FC<{ position: [number,number,number]; color: string }> = ({ position, color }) => {
  ...
  useEffect(() => { if (!dir.current) return; const c = dir.current.shadow.camera as any; c.top=24;c.bottom=-24;c.left=-24;c.right=24;c.far=64; dir.current.shadow.bias=-0.004; }, []);
